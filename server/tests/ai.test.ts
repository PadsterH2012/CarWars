...
// Build an AiContext from the bare enemy list used by the old test signature.
// `allVehicles` = self + enemies so `computeAiInput` can derive enemies itself.
function makeCtx(self: VehicleState, enemies: VehicleState[], skill: number): AiContext {
  return {
    skill,
    allVehicles: [self, ...enemies],
    wreckage: [],
    tick: 0,
  };
}
...
describe('AI driver', () => {
  it('accelerates when target is far away', () => {
    const input = computeAiInput(self, makeCtx(self, [target], 3));
